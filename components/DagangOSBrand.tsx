...
export function DagangOSBrand({ className, compact = false, dark = false, showName = true }: DagangOSBrandProps) {
  return (
    <div className={cn('flex min-w-0 items-start gap-2 leading-none', className)} aria-label={COMPANY.legalName}>
      ...
      <div className={cn('min-w-0 flex flex-col justify-between', compact ? 'h-10' : 'h-12')}>
        <p className={cn('whitespace-nowrap font-black leading-none tracking-[-0.045em]', compact ? 'text-lg' : 'text-2xl', dark ? 'text-slate-100' : 'text-slate-950')}>
        ...
        {showName && (
          <p className={cn('whitespace-nowrap font-semibold leading-none tracking-[0.08em]', compact ? 'text-[7px]' : 'text-[10px]', dark ? 'text-slate-300' : 'text-slate-600')}>
            {COMPANY.legalName}
          </p>
        )}
      </div>
