...
- [ ] **Hermes internal support API** — start and configure the local OpenAI-compatible Hermes service at `127.0.0.1:8642/v1` with `HERMES_API_KEY` and an approved `HERMES_API_MODEL`. The public chat is intentionally pre-sales/read-only only; ChatGPT subscription credentials must not be used as API credentials.
