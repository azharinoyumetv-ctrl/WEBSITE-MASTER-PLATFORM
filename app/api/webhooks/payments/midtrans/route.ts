import { NextResponse } from 'next/server';
import prisma from '@/lib/prisma';
import crypto from 'crypto';
import { decrypt } from '@/lib/crypto';
import { sendOrderConfirmationEmail } from '@/lib/actions/notifications';

export async function POST(req: Request) {
  try {
    const body = await req.json();

    const orderId = body.order_id;
    if (!orderId) {
      return NextResponse.json({ error: 'Missing order_id' }, { status: 400 });
    }

    const order = await prisma.tenantOrder.findUnique({
      where: { id: orderId },
      include: { tenant: { include: { website: true } } }
    });

    if (!order) {
      return NextResponse.json({ error: 'Order not found' }, { status: 404 });
    }

    const website = order.tenant.website;
    let midtransKey = process.env.MIDTRANS_SERVER_KEY;
    if (website?.midtransEncryptedServerKey && website.midtransEncryptedServerKeyIv) {
      midtransKey = decrypt(`${website.midtransEncryptedServerKeyIv}:${website.midtransEncryptedServerKey}`) || midtransKey;
    }

    if (!midtransKey) {
      return NextResponse.json({ error: 'Midtrans server key not configured' }, { status: 500 });
    }

    // Verify signature
    const payload = `${body.order_id}${body.status_code}${body.gross_amount}${midtransKey}`;
    const expectedSignature = crypto.createHash('sha512').update(payload).digest('hex');

    if (body.signature_key !== expectedSignature) {
      return NextResponse.json({ error: 'Invalid signature' }, { status: 403 });
    }

    // Process idempotent webhook
    const idempotencyKey = body.transaction_id || body.order_id;
    if (idempotencyKey) {
      const existingKey = await prisma.paymentIdempotencyKey.findUnique({
        where: { idempotencyKey }
      });
      if (existingKey) {
        return NextResponse.json({ message: 'Already processed' }, { status: 200 });
      }
      await prisma.paymentIdempotencyKey.create({
        data: {
          tenantId: order.tenantId,
          idempotencyKey,
          responsePayload: body,
          expiresAt: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000)
        }
      });
    }

    // Update payment
    const payment = await prisma.tenantPayment.findFirst({
      where: { orderId: order.id }
    });

    if (payment) {
      let status = payment.paymentStatus;
      const transStatus = body.transaction_status;

      if (transStatus === 'capture' || transStatus === 'settlement') {
        status = 'succeeded';
      } else if (transStatus === 'deny' || transStatus === 'cancel' || transStatus === 'expire') {
        status = 'failed';
      } else if (transStatus === 'refund') {
        status = 'refunded';
      } else if (transStatus === 'chargeback') {
        status = 'disputed';
      }

      await prisma.tenantPayment.update({
        where: { id: payment.id },
        data: {
          paymentStatus: status as any,
          externalTransactionId: body.transaction_id || 'unknown',
          metadata: body
        }
      });

      if (transStatus === 'chargeback') {
        const existingDispute = await prisma.tenantPaymentDispute.findFirst({
          where: { tenantId: order.tenantId, paymentId: payment.id }
        });
        if (!existingDispute) {
          await prisma.tenantPaymentDispute.create({
            data: {
              tenantId: order.tenantId,
              paymentId: payment.id,
              status: 'under_review',
              amount: payment.amount,
              reason: 'Midtrans Webhook Chargeback Notification'
            }
          });
        }
      }
    }

    // Update order status
    const transStatus = body.transaction_status;
    if (transStatus === 'capture' || transStatus === 'settlement') {
      await prisma.tenantOrder.update({
        where: { id: order.id },
        data: { 
          orderStatus: 'paid',
          receiptUrl: `/orders/${order.id}/receipt`
        }
      });

      const customerEmail = order.guestEmail || body.payer_email || 'customer@example.com';
      sendOrderConfirmationEmail(order.tenantId, order.id, customerEmail)
        .catch(err => console.error("Failed to send async order confirmation email", err));
    } else if (transStatus === 'deny' || transStatus === 'cancel' || transStatus === 'expire' || transStatus === 'refund') {
      await prisma.tenantOrder.update({
        where: { id: order.id },
        data: { orderStatus: 'cancelled' }
      });

      if (payment) {
        await prisma.tenantPaymentLedger.create({
          data: {
            tenantId: order.tenantId,
            paymentId: payment.id,
            orderId: order.id,
            type: 'reversal',
            amount: payment.amount,
            currency: payment.currency,
            gateway: 'midtrans',
            gatewayTxId: body.transaction_id || 'unknown',
            status: 'failed',
            metadata: body
          }
        });
      }
    }

    return NextResponse.json({ success: true });
  } catch (error: any) {
    console.error('Midtrans webhook error:', error);
    return NextResponse.json({ error: 'Webhook processing failed' }, { status: 500 });
  }
}
