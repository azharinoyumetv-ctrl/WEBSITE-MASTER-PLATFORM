...
function createSystemPolicyPrompt() {
  const catalog = {
    packages: Object.values(packages).map(pkg => ({
      ...
      includedAddons: getIncludedAddonKeys(pkg.key)
        ...
        .filter(Boolean),
      projectSetupUrl: {
        id: `https://wmp.dagangos.com/id/project-setup?package=${pkg.key}`,
        en: `https://wmp.dagangos.com/en/project-setup?package=${pkg.key}`,
      },
    })),
  ...
  return [
    ...
    'Whenever recommending a package, include its exact projectSetupUrl for the visitor language. Never link to dagangos.com; the current storefront host is wmp.dagangos.com.',
