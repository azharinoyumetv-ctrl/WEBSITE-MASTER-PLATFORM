...
export function SupportChatWidget() {
  ...
  return (
    <div className="fixed bottom-24 right-5 z-50 sm:right-8">
      {open && (
        <section aria-label="DagangOS support chat" className="mb-4 flex w-[min(23rem,calc(100vw-2.5rem))] flex-col overflow-hidden rounded-[1.5rem] border border-white/15 bg-slate-950 shadow-[0_28px_80px_rgba(2,6,23,.45)]">
          ...
          <form onSubmit={sendMessage} className="border-t border-white/10 bg-slate-900 p-3">
            <label className="sr-only" htmlFor="dagangos-support-message">Message for DagangOS support</label>
            <div className="flex items-end gap-2 rounded-2xl border border-white/10 bg-white/[.05] p-1.5 focus-within:border-emerald-300/60">
              <textarea id="dagangos-support-message" value={draft} onChange={event => setDraft(event.target.value)} rows={1} maxLength={2000} placeholder="Write a message…" className="min-h-10 flex-1 resize-none bg-transparent px-2 py-2 text-sm text-white outline-none placeholder:text-slate-500" />
              <button type="submit" disabled={!draft.trim() || isSending} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-emerald-400 text-slate-950 transition hover:bg-emerald-300 disabled:cursor-not-allowed disabled:opacity-45" aria-label="Send message">
      ...
      <button type="button" onClick={() => setOpen(current => !current)} aria-expanded={open} aria-label={open ? 'Close support chat' : 'Open support chat'} className="group flex items-center gap-2 rounded-2xl border border-emerald-200/30 bg-slate-950 px-4 py-3 text-sm font-bold text-white shadow-[0_18px_45px_rgba(2,6,23,.35)] transition hover:-translate-y-1 hover:bg-slate-900">
